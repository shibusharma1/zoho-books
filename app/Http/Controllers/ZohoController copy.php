...
class ZohoController extends Controller
{
    ...
    {
        $accessToken = "1000.e88a9341b506b48a83f66e9f3283d813.bada2e38cc710c3c1500439d2ee9d79f";
    ...
    {
        \Log::info('Calling createCustomer with:', $request->all());
        $accessToken = "1000.e88a9341b506b48a83f66e9f3283d813.bada2e38cc710c3c1500439d2ee9d79f";
        // $accessToken = $this->getValidAccessToken();
        $orgId = "917123495";
        // $orgId = session('zoho_organization_id');
        ...
        $zoho_id = $response->json()['contact']['contact_id'] ?? null;

        \Log::info('Zoho Id', [
            'response' => $response->json()['contact']['contact_id']
        ]);

        // $updateResponse = Http::patch("http://127.0.0.1:8000/api/customers/{$id}/zb-id", [
        //     'zb_id' => $zoho_id,
        // ]);

        $updateResponse = Http::asJson()->patch(
        ...
        if ($updateResponse->successful()) {
            ...
            echo $data['message']; // zb_id updated successfully
        } else {
        ...
        if (!$zoho_id) {
            ...
            return response()->json(['error' => 'Zoho customer creation failed']);
        }
